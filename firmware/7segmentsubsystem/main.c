...
static void segmentC(void){

	rcc_periph_clock_enable(RCC_GPIOA); //For PA3
	...
	gpio_clear(GPIOA,GPIO3); //For PA3	
}

static void cleanSegment(void){
	
	gpio_set(GPIOA,GPIO3); //For PA3	
}
...
task1(void *args __attribute__((unused))) {
	...
	for (;;) {
		...
		while ( usb_peek() <= 0 ) {
			...
			value = (value & 0b01111111) | 0b10000000;
			...
			if ( read_flag ) {
				...
				//case 1:
				//	value = values[0]; //Number 0
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 2:
				//	value = values[1]; //Number 1
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 3:
				//	value = values[2]; //Number 2
				//	vTaskDelay(3000);
				//	break;
				//case 4:
				//	value = values[3]; //Number 3
				//	segmentC();	
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 5:
				//	value = 0b00111001; //Number 4
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 6:
				//	value = 0b00110010; //Number 5
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 7:
				//	value = 0b00100010; //Number 6
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 8:
				//	value = 0b01111100; //Number 7
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 9:
				//	value = 0b00100000; //Number 8
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
				//case 10:
				//	value = 0b00110000; //Number 9
				//	segmentC();
				//	vTaskDelay(3000);
				//	cleanSegment();
				//	break;
			...
			value = 0b01100000; //Number 2
			//vTaskDelay(3000);
			for (i = 0; i < 400000; i++)
        			__asm__("nop");


			value = 0b01110000; //Number 3
			segmentC();
			//vTaskDelay(3000);
			for (i = 0; i < 400000; i++)
        			__asm__("nop");
			...
			//value = 0b01111110; // segment a
			//value = 0b01111101; // segment b
			//value = 0b01111011; // segment g
			//value = 0b01110111; // segment d
			//value = 0b01101111; // segment e
			//value = 0b00011111; // segment DP, f
			//value = 0b01011111; // segment DP
			//value = 0b00111111; // segment f
